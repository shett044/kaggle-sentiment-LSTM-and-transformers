...
class SentimentBiLSTM(nn.Module):
    ...
    def encode(
        ...
        enc_hiddens_max = self.dropout(torch.relu(self.enc_max_projection(
            enc_hiddens.max(dim=0).values)))  # batch, 2* hidden size =>  batch,hidden size
        enc_hiddens_min = self.dropout(torch.relu(self.enc_min_projection(
            enc_hiddens.min(dim=0).values)))  # batch, 2* hidden size =>  batch,hidden size
        enc_hiddens_mean = self.dropout(torch.relu(self.enc_avg_projection(
